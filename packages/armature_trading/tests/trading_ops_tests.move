...
use token::cred::CRED;
use triexbook::balance_manager::{Self, BalanceManager, DepositCap, TradeCap, WithdrawCap};
...
use armature_trading::setup_trading_account;
...
fun test_sweep_coin_to_treasury() {
    ...
    test.next_tx(ADMIN);
    {
        ...
        let mut bm = test.take_shared_by_id<BalanceManager>(bm_id);
        let cap_vault = test.take_shared<CapabilityVault>();
        let ticket = make_ticket(dao_id, sweep_coin_to_treasury::new<QUOTE>(bm_id, sweep_amount));
